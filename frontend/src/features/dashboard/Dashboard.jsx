...
const CHART_FONT = { fontSize: 10, fill: 'var(--color-muted)', fontFamily: 'Inter' }
...
const TOOLTIP_LABEL_STYLE = { color: 'var(--color-muted)', marginBottom: 4, fontWeight: 600 }
    ...
    <div style={{ ...TOOLTIP_STYLE, padding: '10px 14px', minWidth: 160 }}>
      ...
      {payload.map((p, i) => (
        <div key={i} style={{ display: 'flex', justifyContent: 'space-between', gap: 16, marginTop: 4 }}>
          ...
          <span style={{ fontWeight: 600, color: 'var(--color-ink)', fontSize: '0.75rem' }}>{Math.round(p.value)}{p.name.includes('%') ? '%' : ''}</span>
        </div>
      ))}
    </div>
  )
    ...
    <div style={{ ...TOOLTIP_STYLE, padding: '10px 14px', minWidth: 180 }}>
      ...
      {stress != null && (
        <div style={{ display: 'flex', justifyContent: 'space-between', gap: 16, marginTop: 4 }}>
          <span style={{ color: 'var(--color-muted)', fontSize: '0.75rem' }}>Stress Score</span>
          <span style={{ fontWeight: 600, color: stressColor(stress), fontSize: '0.75rem' }}>{Math.round(stress)} — {level}</span>
        </div>
      )}
      {settle != null && (
        <div style={{ display: 'flex', justifyContent: 'space-between', gap: 16, marginTop: 4 }}>
          <span style={{ color: 'var(--color-muted)', fontSize: '0.75rem' }}>Settlement %</span>
          <span style={{ fontWeight: 600, color: 'var(--color-amber)', fontSize: '0.75rem' }}>{Math.round(settle)}%</span>
        </div>
      )}
    </div>
  )
  ...
  return (
    <ResponsiveContainer width="100%" height={220}>
      <LineChart data={data} margin={{ top: 8, right: 16, left: -18, bottom: 4 }}>
        ...
        <ReferenceLine y={75} stroke="var(--color-danger)" strokeDasharray="3 3" strokeOpacity={0.4} label={{ value: 'High', position: 'right', fontSize: 9, fill: 'var(--color-danger)' }} />
        <ReferenceLine y={50} stroke="var(--color-warning)" strokeDasharray="3 3" strokeOpacity={0.3} label={{ value: 'Med', position: 'right', fontSize: 9, fill: 'var(--color-warning)' }} />
        <Tooltip content={<StressTooltip />} />
        <Line type="monotone" dataKey="Stress Score" stroke="var(--color-ink)" strokeWidth={2.5}
          dot={{ r: 4, fill: 'var(--color-ink)', strokeWidth: 0 }} activeDot={{ r: 6, fill: 'var(--color-ink)' }} />
        <Line type="monotone" dataKey="Settlement %" stroke="var(--color-amber)" strokeWidth={2.5} strokeDasharray="5 3"
          dot={{ r: 4, fill: 'var(--color-amber)', strokeWidth: 0 }} activeDot={{ r: 6, fill: 'var(--color-amber)' }} />
  ...
  return (
    <ResponsiveContainer width="100%" height={220}>
      <BarChart data={data} margin={{ top: 8, right: 16, left: -18, bottom: 4 }} barGap={4}>
        ...
        <XAxis dataKey="name" axisLine={false} tickLine={false} tick={CHART_FONT} dy={8} />
        <YAxis axisLine={false} tickLine={false} tick={CHART_FONT} domain={[0, 100]} />
        <ReferenceLine y={75} stroke="var(--color-danger)" strokeDasharray="3 3" strokeOpacity={0.35} />
        <ReferenceLine y={50} stroke="var(--color-warning)" strokeDasharray="3 3" strokeOpacity={0.25} />
        ...
        <Bar dataKey="settlement_percentage" name="Settlement %" fill="var(--color-amber)" fillOpacity={0.55} radius={[3, 3, 0, 0]} maxBarSize={32} />
        <Legend wrapperStyle={{ fontSize: '0.6875rem', paddingTop: 12, fontFamily: 'Inter', color: '#7A776E' }} />
      </BarChart>
    </ResponsiveContainer>
  )
...
function LoanRadarChart({ loans }) {
  ...
  const RADAR_COLORS = ['var(--color-ink)', 'var(--color-amber)', 'var(--color-green)', 'var(--color-danger)', 'var(--color-info)', 'var(--color-warning)']
  ...
  return (
    <div style={{ maxWidth: 1240, margin: '0 auto' }} className="fade-in page-content">
      ...
      <div className="dashboard-grid" style={{ marginBottom: 24 }}>
        {/* Insights */}
        <div style={{ background: 'var(--color-amber-dim)', border: '1px solid var(--color-border-amber)', borderRadius: 'var(--radius-md)', padding: '24px', display: 'flex', flexDirection: 'column', justifyContent: 'space-between', gap: 20 }}>
          <div>
        ...
        {activeChart === 'trend' && !loading && snapshots.length >= 2 && (
          <div style={{ padding: '0 20px 16px', display: 'flex', gap: 20, flexWrap: 'wrap' }}>
            <span style={{ display: 'flex', alignItems: 'center', gap: 6, fontSize: '0.6875rem', color: 'var(--color-muted)' }}>
              <span style={{ display: 'inline-block', width: 16, height: 2.5, background: 'var(--color-ink)', borderRadius: 1 }} /> Stress Score
            </span>
            <span style={{ display: 'flex', alignItems: 'center', gap: 6, fontSize: '0.6875rem', color: 'var(--color-muted)' }}>
              <span style={{ display: 'inline-block', width: 16, height: 0, borderTop: '2.5px dashed var(--color-amber)' }} /> Settlement %
            </span>
            <span style={{ display: 'flex', alignItems: 'center', gap: 6, fontSize: '0.6875rem', color: 'var(--color-muted)' }}>
              <span style={{ display: 'inline-block', width: 16, height: 0, borderTop: '2px dashed var(--color-danger)', opacity: 0.5 }} /> High threshold (75)
            </span>
          </div>
        )}
